...
export class PanelLayoutManager implements AppModule {
  ...
  renderLayout(): void {
      ...
      <div class="header">
        <div class="header-left">
          ...
          <div class="brand-logo">
            <svg class="brand-icon" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            ...
            <span class="brand-name">AST <span class="brand-accent">STRATEGIE</span></span>
          ...
          <div class="variant-switcher">
            <button class="variant-option ${SITE_VARIANT === 'full' ? 'active' : ''}" data-variant="full" title="${t('header.world')}">
              <span class="variant-icon">🌍</span>
              <span class="variant-label">${t('header.world')}</span>
            </button>
            <span class="variant-divider"></span>
            <button class="variant-option ${SITE_VARIANT === 'tech' ? 'active' : ''}" data-variant="tech" title="${t('header.tech')}">
              <span class="variant-icon">💻</span>
              <span class="variant-label">${t('header.tech')}</span>
            </button>
            <span class="variant-divider"></span>
            <button class="variant-option ${SITE_VARIANT === 'finance' ? 'active' : ''}" data-variant="finance" title="${t('header.finance')}">
              <span class="variant-icon">📈</span>
          ...
          <div class="status-indicator">
            ...
            <span class="status-live-text">${t('header.live')}</span>
          </div>

          <!-- Region Selector -->
          <div class="region-selector">
            <svg class="region-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="header-right">
          ...
          <!-- Search -->
          <button class="hdr-btn search-btn" id="searchBtn" title="${t('header.search')}">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
              <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
            </svg>
            <span class="hdr-btn-label">${t('header.search')}</span>
            <kbd class="search-kbd">⌘K</kbd>
          </button>

          ${this.ctx.isDesktopApp ? '' : `
          <!-- Copy Link -->
          <button class="hdr-btn copy-link-btn" id="copyLinkBtn" title="${t('header.copyLink')}">
          ...
          <!-- Theme Toggle -->
          <button class="hdr-btn theme-toggle-btn" id="headerThemeToggle" title="${t('header.toggleTheme')}">
        ...
        : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>'}
          ...
          ${this.ctx.isDesktopApp ? '' : `
          <!-- Fullscreen -->
          <button class="hdr-btn fullscreen-btn" id="fullscreenBtn" title="${t('header.fullscreen')}">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <!-- Settings -- mounted by EventHandlerManager into this span -->
    ...
    // Variant switching
    const variantButtons = this.ctx.container.querySelectorAll('.variant-option');
    variantButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        const v = (btn as HTMLElement).dataset.variant;
        if (v && v !== SITE_VARIANT) {
          localStorage.setItem('worldmonitor-variant', v);
